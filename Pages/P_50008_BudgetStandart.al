page 50008 "Budget Standard"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                //>>>>
                field(BudgetType; BudgetType)
                {
                    ApplicationArea = suite;
                    Caption = 'Budget Type';
                }
                field(BudgetStatus; BudgetStatus)
                {
                    ApplicationArea = suite;
                ...
                field(FinancialYear; FinancialYear)
                {
                    ...
                    var
                        year: Integer;
                    begin
                        Evaluate(year, FinancialYear);
                        DateFilter := FinancialYear + '-07-01..' + format(year + 1) + '-06-30';
                    end;
                }
                field(budget_Name; budget_Name)
                {
                    ApplicationArea = all;
            ...
            part(MatrixForm; "Budget Matrix Standard")
            {
                ApplicationArea = Suite;
            }
            group(Filters)
            {
                ...
                field(BudgetDim5Filter; BudgetDim5Filter)
                {
                    ApplicationArea = Dimensions;
                    CaptionClass = GetCaptionClass(5);
                    Caption = 'Budget Dimension 5 Filter';
                    Enabled = BudgetDim5FilterEnable;
                    ToolTip = 'Specifies a filter by a budget dimension. You can specify four additional dimensions on each budget that you create.';
                    ...
                    var
                        DimensionValue: Record "Dimension Value";
                    begin
                        exit(DimensionValue.LookUpDimFilter(GLBudgetName."Budget Dimension 5 Code", Text));
                    end;

                    trigger OnValidate()
                    begin
                        BudgetDim5FilterOnAfterValidate();
                    end;
                }
                field(BudgetDim6Filter; BudgetDim6Filter)
                {
                    ApplicationArea = Dimensions;
                    CaptionClass = GetCaptionClass(6);
                    Caption = 'Budget Dimension 6 Filter';
                    Enabled = BudgetDim6FilterEnable;
                    ToolTip = 'Specifies a filter by a budget dimension. You can specify four additional dimensions on each budget that you create.';
                    ...
                    var
                        DimensionValue: Record "Dimension Value";
                    begin
                        exit(DimensionValue.LookUpDimFilter(GLBudgetName."Budget Dimension 6 Code", Text));
                    end;

                    trigger OnValidate()
                    begin
                        BudgetDim6FilterOnAfterValidate();
                    end;
                }
                field(BudgetDim7Filter; BudgetDim7Filter)
                {
                    ApplicationArea = Dimensions;
                    CaptionClass = GetCaptionClass(7);
                    Caption = 'Budget Dimension 7 Filter';
                    Enabled = BudgetDim7FilterEnable;
                    ToolTip = 'Specifies a filter by a budget dimension. You can specify four additional dimensions on each budget that you create.';
                    ...
                    var
                        DimensionValue: Record "Dimension Value";
                    begin
                        exit(DimensionValue.LookUpDimFilter(GLBudgetName."Budget Dimension 7 Code", Text));
                    end;

                    trigger OnValidate()
                    begin
                        BudgetDim7FilterOnAfterValidate();
                    end;
                }
                field(BudgetDim8Filter; BudgetDim8Filter)
                {
                    ApplicationArea = Dimensions;
                    ...
                    var
                        DimensionValue: Record "Dimension Value";
                    begin
                        exit(DimensionValue.LookUpDimFilter(GLBudgetName."Budget Dimension 8 Code", Text));
                    end;

                    trigger OnValidate()
                    begin
                        BudgetDim8FilterOnAfterValidate();
                    end;
                }
            }
        }
            ...
            group("F&unctions")
            {
                ...
                action("Export to Excel")
                {
                    ...
                    begin
                        ...
                        //neha
                        // GLBudgetEntry.SetFilter("Budget Dimension 5 Code", BudgetDim1Filter);
                        // GLBudgetEntry.SetFilter("Budget Dimension 6 Code", BudgetDim2Filter);
                        // GLBudgetEntry.SetFilter("Budget Dimension 7 Code", BudgetDim3Filter);
                        // GLBudgetEntry.SetFilter("Budget Dimension 8 Code", BudgetDim4Filter);
    ...
    begin
        ...
        ///neha
        BudgetDim5FilterEnable := true;
        BudgetDim6FilterEnable := true;
        BudgetDim7FilterEnable := true;
        BudgetDim8FilterEnable := true;
    end;
    ...
    var
        ...
        Text012: Label '1,6,,Budget Dimension 5 Filter';
        Text013: Label '1,6,,Budget Dimension 6 Filter';
        Text014: Label '1,6,,Budget Dimension 7 Filter';
        Text015: Label '1,6,,Budget Dimension 8 Filter';
        ...
        ///
        BudgetDim5FilterEnable: Boolean;
        BudgetDim6FilterEnable: Boolean;
        BudgetDim7FilterEnable: Boolean;
        BudgetDim8FilterEnable: Boolean;

    protected var
        GLAccBudgetBuf: Record "G/L Acc. Budget Buffer";
        BudgetType: Option " ","Initial Budget","Revised Budget","Approval Pending","Final Budget";
        BudgetStatus: text[30];
        //
        FinancialYear: text[50];
        budget_Name: text[100];
        BudgetDim5Filter: Text;
        BudgetDim6Filter: Text;
        BudgetDim7Filter: Text;
        BudgetDim8Filter: Text;
        ///
        GLBudget: Boolean;
        BudgetName: Code[10];
    ...
    begin
        ...
        case BudgetDimType of
            ...
            5:
                begin
                    if GLBudgetName."Budget Dimension 5 Code" <> '' then
                        exit('1,6,' + GLBudgetName."Budget Dimension 5 Code");

                    exit(Text012);
                end;
            6:
                begin
                    if GLBudgetName."Budget Dimension 6 Code" <> '' then
                        exit('1,6,' + GLBudgetName."Budget Dimension 6 Code");

                    exit(Text013);
                end;
            7:
                begin
                    if GLBudgetName."Budget Dimension 7 Code" <> '' then
                        exit('1,6,' + GLBudgetName."Budget Dimension 7 Code");

                    exit(Text014);
                end;
            8:
                begin
                    if GLBudgetName."Budget Dimension 8 Code" <> '' then
                        exit('1,6,' + GLBudgetName."Budget Dimension 8 Code");
    ...
    local procedure BudgetDim5FilterOnAfterValidate()
    begin
        GLAccBudgetBuf.SetFilter("Budget Dimension 5 Filter", BudgetDim5Filter);
        if ColumnDimType = ColumnDimType::"Budget Dimension 5" then
            GenerateColumnCaptions("Matrix Page Step Type"::Initial);
        UpdateMatrixSubform();
    end;

    local procedure BudgetDim6FilterOnAfterValidate()
    begin
        GLAccBudgetBuf.SetFilter("Budget Dimension 6 Filter", BudgetDim6Filter);
        if ColumnDimType = ColumnDimType::"Budget Dimension 6" then
            GenerateColumnCaptions("Matrix Page Step Type"::Initial);
        UpdateMatrixSubform();
    end;

    local procedure BudgetDim7FilterOnAfterValidate()
    begin
        GLAccBudgetBuf.SetFilter("Budget Dimension 7 Filter", BudgetDim7Filter);
        if ColumnDimType = ColumnDimType::"Budget Dimension 7" then
            GenerateColumnCaptions("Matrix Page Step Type"::Initial);
        UpdateMatrixSubform();
    end;

    local procedure BudgetDim8FilterOnAfterValidate()
    begin
        GLAccBudgetBuf.SetFilter("Budget Dimension 8 Filter", BudgetDim8Filter);
        if ColumnDimType = ColumnDimType::"Budget Dimension 8" then
            GenerateColumnCaptions("Matrix Page Step Type"::Initial);
        UpdateMatrixSubform();
    end;
